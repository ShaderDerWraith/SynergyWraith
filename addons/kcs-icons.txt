...
(function() {
    ...
    // --- POCZĄTEK TWOJEJ LISTY MONSTERMAPPINGS ---
    // UPEWNIJ SIĘ, ŻE WKLEIŁEŚ TUTAJ SWOJĄ PEŁNĄ LISTĘ
    ...
    const CACHE_KEY = 'kcsMonsterIconCache_v0.1'; // Klucz cache pozostaje ten sam, przechowuje itemID -> imgUrl
    ...
    function addMonsterIcon(itemElement, monsterImgUrl) {
        ...
        if (existingIcon) {
            if (existingIcon.src === monsterImgUrl) {
                itemElement.dataset.monsterIconAdded = 'true'; // Upewnij się, że atrybut jest ustawiony
                return; // Poprawna ikona już istnieje
            } else {
                console.log(`[KCS Icons] Zmieniono URL ikony dla itemu. Aktualizowanie.`);
                existingIcon.remove(); // Usuń starą/inną ikonę
            }
        }
        ...
        img.style.right = '0px';
        img.style.width = '32px'; // Rozmiar ikony potwora
        img.style.height = '32px';
        img.style.zIndex = '10'; // Ikona potwora
        ...
        if (cooldownDiv) {
            ...
            cooldownDiv.style.zIndex = '11'; // Wyżej niż ikona potwora
            // console.log(`[KCS Icons] Ustawiono z-index dla cooldownDiv na itemie ${itemElement.className.match(/item-id-(\d+)/)?.[1]}`);
        }
        ...
        if (amountDiv) {
            ...
            amountDiv.style.zIndex = '11'; // Wyżej niż ikona potwora
            // console.log(`[KCS Icons] Ustawiono z-index dla amountDiv na itemie ${itemElement.className.match(/item-id-(\d+)/)?.[1]}`);
        }

        itemElement.dataset.monsterIconAdded = 'true';
        const itemIdForLog = itemElement.className.match(/item-id-(\d+)/)?.[1] || 'nieznany';
        // console.log(`[KCS Icons] Dodano/zaktualizowano ikonę ${monsterImgUrl} do przedmiotu ${itemIdForLog}`);
    }
                ...
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === Node.ELEMENT_NODE && node.classList.contains('tip-wrapper')) {
                        ...
                        if (!(itemName.includes("Kamień Czerwonego Smoka") || itemName.includes("Zwój Czerwonego Smoka") || itemName.includes("Ulotny zwój czerwonego smoka"))) {
                            // console.log(`[KCS Icons] Przedmiot "${itemName}" nie jest KCS ani Zwojem. Ignorowanie.`);
                            return;
                        }
                        // console.log(`[KCS Icons] Wykryto tooltip dla: ${itemName}`);
    ...
    function applyIconsFromCache() {
        ...
        if (Object.keys(cache).length === 0) {
            // console.log('[KCS Icons] Cache jest pusty, nic do załadowania przy starcie.');
            return;
        }
        // console.log('[KCS Icons] Próba załadowania ikon z cache:', cache);
        const allItems = document.querySelectorAll('.item[class*="item-id-"]');
        // console.log(`[KCS Icons] Znaleziono ${allItems.length} przedmiotów z klasą item-id. Sprawdzanie cache...`);
        allItems.forEach(itemElement => {
            ...
            if (itemId && cache[itemId]) {
                // console.log(`[KCS Icons] Stosowanie ikony z cache dla przedmiotu ${itemId}`);
                addMonsterIcon(itemElement, cache[itemId]);
            }
        });
        // console.log('[KCS Icons] Zakończono stosowanie ikon z cache.');
    }
        ...
        for (const mutation of mutationsList) {
            if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                ...
                if (potentiallyNewItems) {
                    // console.log('[KCS Icons] Wykryto potencjalnie nowe przedmioty w DOM, ponowne stosowanie z cache.');
                    applyIconsFromCache();
                }
            }
        }
    ...
    // Inicjalizacja w stylu Twojego skryptu v0.3 / mojej v0.5
    // console.log('[KCS Icons] Inicjalizacja skryptu v0.6...');

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            // console.log('[KCS Icons] DOMContentLoaded - uruchamianie applyIconsFromCache.');
            applyIconsFromCache();
        });
    } else {
        // console.log('[KCS Icons] DOM już załadowany - uruchamianie applyIconsFromCache.');
        applyIconsFromCache();
    }

    tooltipObserver.observe(document.body, { childList: true, subtree: true });
    // console.log('[KCS Icons] TooltipObserver aktywny.');

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            // console.log('[KCS Icons] DOMContentLoaded - uruchamianie DynamicItemObserver.');
            dynamicItemObserver.observe(document.body, { childList: true, subtree: true });
            // console.log('[KCS Icons] DynamicItemObserver aktywny (po DOMContentLoaded).');
        });
    } else {
        // console.log('[KCS Icons] DOM już załadowany - uruchamianie DynamicItemObserver.');
        dynamicItemObserver.observe(document.body, { childList: true, subtree: true });
        // console.log('[KCS Icons] DynamicItemObserver aktywny.');
    }
